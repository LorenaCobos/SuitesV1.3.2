...
//    }
  ...
  @RequestMapping(value = "/s/detalleEventoAdmin", method = RequestMethod.POST)
    ...
    @RequestMapping(value = "/s/eventos", method = { RequestMethod.GET, RequestMethod.POST })
    public ModelAndView eventos(@ModelAttribute("command") EventoSeleccionadoEntidad eventoSeleccionado, HttpSession session, HttpServletResponse response, HttpServletRequest req) throws ExceptionCmm, Exception {
        UsuariosEntidad usuario = session.getAttribute("usuarioEntidad") == null ? new UsuariosEntidad() : (UsuariosEntidad) session.getAttribute("usuarioEntidad");
       session.setMaxInactiveInterval(Integer.parseInt(environment.getProperty("sessionTimeOut", "1800")));
        if(session.getAttribute("usuarioEntidad") == null)
        {
        ...
        else
        {
            ...
            if (recintos != null && eventoSeleccionado.getRecintoBusquedaId() != 0) {
                modeloVista.addObject("suites", catalogo.getSuitesUsuario(eventoSeleccionado.getRecintoBusquedaId(), 0, usuario.getUsuarioId()));
            } else if (recintos != null && recintos.size() == 1) {
