package com.sb.suites.dwr.utils;

import sun.misc.BASE64Decoder;
/**
 *
 * @author Ricardo Uriel Muñoz
 */
public class AesDwr {
    ...
    public static String decrypt(String ciphertext) throws Exception {
      ...
      int nBits = 128;
      String password ="XmdukHGRv45cDF";
      ...
      for (int i = 0; i < decodedCipher.length; i++) {
            decoded[i] = decodedCipher[i] < 0 ? (short)(decodedCipher[i] + 256) : decodedCipher[i];
      }
      ...
      for (int b=0; b<nBlocks; b++) {
            ...
            for (int ind = 8+b*blockSize; ind < 8+b*blockSize+blockSize; ind++) {
      ...
      for (int b=0; b<nBlocks; b++) {
        ...
        short[] cipherCntr = cipher(counterBlockTmp, keySchedule);  // encrypt counter block
        ...
        for (int i=0; i<plaintxtShort.length; i++) {
            if (plaintxtShort[i] > 194) {
                    plaintext.append((char)((plaintxtShort[i] - 194) * 64 + plaintxtShort[i+1]));
                    i++;
          } else {
    ...
    private static short[] keyExpansion(short[] key) {  // generate Key Schedule (byte-array Nr+1 x Nb) from Key [§5.2]
    ...
    private static short[] subWord(short[] w) {    // apply SBox to 4-byte word w
      for (int i=0; i<4; i++) {
    ...
    private static short[] rotWord(short[] w) {    // rotate 4-byte word w left by one byte
      short tmp = w[0];
    ...
    private static short convertHexToShort(String hex) {
            ...
            if (hex != null && hex.length() == 4 && hex.startsWith("0x")
                            && HEX.indexOf(hex.charAt(2)) > -1 && HEX.indexOf(hex.charAt(3)) > -1)
            {
    ...
    private static byte convertCharToByte(char c) {
            return (c >= 48 && c <= 57) ? (byte)(((int)c) - 48) : (byte)(((int)c) - 97 + 10);
    }
    ...
    private static short[] cipher(short[] input, short[] w) {    // main Cipher function [§5.1]
      int Nb = 4;               // block size (in words): no of columns in state (fixed at 4 for AES)
      int Nr = (w.length/Nb - 1) / 4; // no of rounds: 10/12/14 for 128/192/256-bit keys
      ...
      short[] output = new short[4*Nb];  // convert state to 1-d array before returning [§3.4]
      for (int i=0; i<4*Nb; i++) {
    ...
    private static short[] addRoundKey(short[] state, short[] w, int rnd, int Nb) {  // xor Round Key into state S [§5.1.4]
      for (int r=0; r<4; r++) {
        for (int c=0; c<Nb; c++) state[r*4+c] ^= w[(rnd*4+c)*4+r];
      }
      return state;
    }

    private static short[] subBytes(short[] s, int Nb) {    // apply SBox to state S [§5.1.1]
      for (int r=0; r<4; r++) {
        for (int c=0; c<Nb; c++) s[r*4+c] = convertHexToShort(sBox[s[r*4+c]]);
      }
      return s;
    }

    private static short[] shiftRows(short[] s, int Nb) {    // shift row r of state S left by r bytes [§5.1.2]
      short[] t = new short[4];
      for (int r=1; r<4; r++) {
        for (int c=0; c<4; c++) t[c] = s[r*4+(c+r)%Nb];  // shift into temp copy
        for (int c=0; c<4; c++) s[r*4+c] = t[c];         // and copy back
      }          // note that this will work for Nb=4,5,6, but not 7,8 (always 4 for AES):
      return s;  // see asmaes.sourceforge.net/rijndael/rijndaelImplementation.pdf
    }

    private static short[] mixColumns(short[] s, int Nb) {   // combine bytes of each col of state S [§5.1.3]
      for (int c=0; c<4; c++) {
        short[] a = new short[4];  // 'a' is a copy of the current column from 's'
        short[] b = new short[4];  // 'b' is a•{02} in GF(2^8)
        for (int i=0; i<4; i++) {
          a[i] = s[i*4+c];
          b[i] = ((s[i*4+c]&0x80) > 0) ? (short)(s[i*4+c]<<1 ^ 0x011b) : (short)(s[i*4+c]<<1);

        }
        // a[n] ^ b[n] is a•{03} in GF(2^8)
        s[0*4+c] = (short)(b[0] ^ a[1] ^ b[1] ^ a[2] ^ a[3]); // 2*a0 + 3*a1 + a2 + a3
        s[1*4+c] = (short)(a[0] ^ b[1] ^ a[2] ^ b[2] ^ a[3]); // a0 * 2*a1 + 3*a2 + a3
        s[2*4+c] = (short)(a[0] ^ a[1] ^ b[2] ^ a[3] ^ b[3]); // a0 + a1 + 2*a2 + 3*a3
        s[3*4+c] = (short)(a[0] ^ b[0] ^ a[1] ^ a[2] ^ b[3]); // 3*a0 + a1 + a2 + 2*a3
      }
      return s;
    }
